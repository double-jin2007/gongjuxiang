...
class AdminPosService
{
    ...
    public function update($admin_id, string $pos_id = ''): bool
    {
        ...
        Db::table('tp_admin_pos')->where('admin_id', $admin_id)->delete();
        if (!$pos_id) return true;
        $pos_id = explode(',',$pos_id);
        foreach ($pos_id as $role){
            Db::table('tp_admin_pos')->insert(['admin_id' => $admin_id, 'pos_id' => $role]);
        }
        return true;
    }
    ...
    public function getPosByAdmin($admin_id, $showPos = false)
    {
        ...
        try {
            $info = Db::table('tp_admin_pos')->where('admin_id', $admin_id)->find();
    ...
    public function deleteByPos($pos_id): bool
    {
        if ($pos_id) {
            try {
                Db::table('tp_admin_pos')->where('pos_id', $pos_id)->delete();
            } catch (\Exception $exception) {
                return false;
            }
            return true;
