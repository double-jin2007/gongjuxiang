...
class AdminRoleService
{
    ...
    public function update($admin_id, string $role_id = ''): bool
    {
        ...
        Db::table('tp_admin_role')->where('admin_id', $admin_id)->delete();
        if (!$role_id) return true;
        $role_id = explode(',',$role_id);
        foreach ($role_id as $role){
            Db::table('tp_admin_role')->insert(['admin_id' => $admin_id, 'role_id' => $role]);
        }
        return true;
    }
    ...
    public function getRoleByAdmin($admin_id, $showRole = false): array
    {
        ...
        try {
            $list = Db::table('tp_admin_role')->where('admin_id', $admin_id)->select()->toArray();
    ...
    public function getAdminIdByRoleId($role_id): array
    {
        ...
        try {
            $list = Db::table('tp_admin_role')->where('role_id', $role_id)->select()->toArray();
        } catch (\Exception $exception) {
            return [];
        }
    ...
    public function deleteByRole($role_id): bool
    {
        if ($role_id) {
            try {
                Db::table('tp_admin_role')->where('role_id', $role_id)->delete();
            } catch (\Exception $exception) {
                return false;
            }
            return true;
