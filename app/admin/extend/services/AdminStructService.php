...
class AdminStructService
{
    ...
    public function update($admin_id, $struct_id): bool
    {
        ...
        Db::table('tp_admin_struct')->where('admin_id', $admin_id)->delete();
        if (!$struct_id) return true;
        Db::table('tp_admin_struct')->insert(['admin_id' => $admin_id, 'struct_id' => $struct_id]);
        return true;
    }
    ...
    public function getStructByAdminId($admin_id, $showStruct = false)
    {
        if (!$admin_id) return false;
        try {
            $info = Db::table('tp_admin_struct')->where('admin_id', $admin_id)->find();
    ...
    public function getAdminIdByStructId($struct_id): array
    {
        ...
        try {
            if(is_array($struct_id)){
                $list = Db::table('tp_admin_struct')->whereIn('struct_id', $struct_id)->select()->toArray();
            }else{
                $list = Db::table('tp_admin_struct')->where('struct_id', $struct_id)->select()->toArray();
            }
        } catch (\Exception $exception) {
            return [];
        }
    ...
    public function deleteByStruct($struct_id): bool
    {
        if ($struct_id) {
            try {
                Db::table('tp_admin_struct')->where('struct_id', $struct_id)->delete();
            } catch (\Exception $exception) {
                return false;
            }
            return true;
