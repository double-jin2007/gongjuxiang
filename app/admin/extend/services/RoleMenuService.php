...
class RoleMenuService
{
    ...
    public function update($role_id, $treeId = null):bool
    {
        ...
        Db::name('tp_role_menu')->insertAll($data);
        return true;
    }
    ...
    public function getRoleMenuList($role_id): array
    {
        ...
        if(is_array($role_id)){
            foreach ($role_id as $role){
                try {
                    $chList = Db::table('tp_role_menu')->where('role_id', $role)->select()->toArray();
            ...
            try {
                $list = Db::table('tp_role_menu')->where('role_id', $role_id)->select()->toArray();
            } catch (\Exception $exception) {
    ...
    public function deleteByRole($role_id):bool{
        if($role_id){
            try {
                Db::table('tp_role_menu')->where('role_id', $role_id)->delete();
            }catch (\Exception $exception){
                return false;
            }
            return true;
    ...
    public function deleteByMenu($menu_id):bool{
        if($menu_id){
            try {
                Db::table('tp_role_menu')->where('menu_id', $menu_id)->delete();
            }catch (\Exception $exception){
                return false;
            }
            return true;
