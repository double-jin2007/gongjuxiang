...
class RoleStructService
{
    ...
    public function update($role_id, $struct_id = null): bool
    {
        ...
        Db::table('tp_role_struct')->insertAll($data);
        return true;
    }
    ...
    public static function getRoleStructList($roleId): array
    {
        ...
        if ($roleId) {
            try {
                if(is_array($roleId)){
                    $list = Db::table('tp_role_struct')->whereIn('role_id', $roleId)->select()->toArray();
                }else{
                    $list = Db::table('tp_role_struct')->where('role_id', $roleId)->select()->toArray();
                }
                if ($list) {
    ...
    public function deleteByRole($role_id):bool{
        if($role_id){
            try {
                Db::table('tp_role_struct')->where('role_id', $role_id)->delete();
            } catch (\Exception $exception) {
                return false;
            }
            return true;
    ...
    public function deleteByStruct($struct_id):bool{
        if($struct_id){
            try {
                Db::table('tp_role_struct')->where('struct_id', $struct_id)->delete();
            } catch (\Exception $exception) {
                return false;
            }
            return true;
